...
def cpes():
    ...
    if request.base_url.endswith(".csv/"):
        writer = CsvWriter('cpes',columns=NodeCpe.export_columns())
        return send_file(writer.write(query,format={'status': lambda value: NODE_STATUS_DICT.get(value)}),as_attachment=True,attachment_filename='cpes.csv')
    else:
